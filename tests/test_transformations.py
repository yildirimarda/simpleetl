...
def test_filter_data():
    ...
    with pytest.raises(
        ValueError, match="Either filter_func or column must be provided"
    ):
        filter_data(df)
...
def test_map_values():
    ...
    with pytest.raises(
        TypeError, match="Mapping must be a dictionary or a callable function"
    ):
...
def test_aggregate_data():
    ...
    with pytest.raises(
        ValueError, match="Groupby column 'nonexistent' not found in DataFrame"
    ):
        aggregate_data(df, "nonexistent", {"salary": "mean"})

    # Test with non-existent aggregation column
    with pytest.raises(
        ValueError, match="Aggregation column 'nonexistent' not found in DataFrame"
    ):
