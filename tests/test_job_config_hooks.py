...
class TestNewHookWiring:
    def test_metrics_hook_registered_when_enabled(self, frame):
        ...
        job = PassthroughJob(config, frame)
        assert any(isinstance(h, MetricsHook) for hooks in job._config_hooks.values() for h in hooks)
    ...
    def test_lineage_hook_registered_when_openlineage_url_set(self, frame):
        ...
        job = PassthroughJob(config, frame)
        assert any(isinstance(h, LineageHook) for hooks in job._config_hooks.values() for h in hooks)
    ...
    def test_provenance_hook_registered_when_enabled(self, frame):
        ...
        job = PassthroughJob(config, frame)
        assert any(isinstance(h, ProvenanceHook) for hooks in job._config_hooks.values() for h in hooks)
    ...
    def test_quality_check_hook_registered_when_quality_checks_configured(self, frame):
        ...
        job = PassthroughJob(config, frame)
        assert any(isinstance(h, QualityCheckHook) for hooks in job._config_hooks.values() for h in hooks)

    def test_no_new_hooks_registered_by_default(self, frame):
        job = PassthroughJob(_config(), frame)
        for hooks in job._config_hooks.values():
            assert not any(isinstance(h, (MetricsHook, LineageHook, ProvenanceHook, QualityCheckHook)) for h in hooks)
