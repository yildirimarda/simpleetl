...
class TestWindowFunctions:
    ...
    def test_rank(self):
        ...
        result = window_functions(
            df, "dept", "salary", {"sal_rank": {"function": "rank"}}
        )
    ...
    def test_dense_rank(self):
        ...
        result = window_functions(
            df, "dept", "salary", {"dense_r": {"function": "dense_rank"}}
        )
    ...
    def test_row_number(self):
        ...
        result = window_functions(
            df, "dept", "salary", {"rn": {"function": "row_number"}}
        )
    ...
    def test_percent_rank(self):
        ...
        result = window_functions(
            df, "dept", "salary", {"pct_r": {"function": "percent_rank"}}
        )
    ...
    def test_cume_dist(self):
        ...
        result = window_functions(
            df, "dept", "salary", {"cume": {"function": "cume_dist"}}
        )
    ...
    def test_invalid_function(self):
        ...
        with pytest.raises(ValueError, match="Invalid window function"):
            window_functions(df, "dept", "salary", {"x": {"function": "invalid_func"}})

    def test_missing_partition_column(self):
        """Test that a missing partition column raises ValueError."""
        df = pd.DataFrame({"dept": ["A"], "salary": [50000]})
        with pytest.raises(
            ValueError, match="Partition column 'nonexistent' not found"
        ):
            window_functions(
                df, "nonexistent", "salary", {"rn": {"function": "row_number"}}
            )
    ...
    def test_missing_order_column(self):
        ...
        with pytest.raises(ValueError, match="Order column 'nonexistent' not found"):
            window_functions(
                df, "dept", "nonexistent", {"rn": {"function": "row_number"}}
            )
    ...
    def test_lag_without_column_raises(self):
        ...
        with pytest.raises(ValueError, match="requires a 'column' key"):
            window_functions(df, "dept", "salary", {"x": {"function": "lag"}})

    def test_empty_dataframe(self):
        """Test window functions on an empty DataFrame."""
        df = pd.DataFrame(
            {"dept": pd.Series([], dtype=str), "salary": pd.Series([], dtype=float)}
        )
        result = window_functions(
            df, "dept", "salary", {"rn": {"function": "row_number"}}
        )
    ...
    def test_list_partition_and_order(self):
        ...
        result = window_functions(
            df, ["dept", "team"], "salary", {"rn": {"function": "row_number"}}
        )
...
class TestStringOperations:
    ...
    def test_regex_extract_missing_pattern(self):
        ...
        with pytest.raises(
            ValueError, match="'regex_extract' operation requires 'pattern'"
        ):
...
class TestDateOperations:
    ...
    def test_trunc_month(self):
        ...
        assert (
            result["dt"].iloc[0] == pd.Timestamp("2024-02-29")
            or result["dt"].iloc[0].month == 2
            or result["dt"].iloc[0].month == 3
        )
...
class TestTransformationChain:
    ...
    def test_sort_and_limit(self):
        ...
        result = TransformationChain(df).sort(by="name").limit(2).result()
        assert len(result) == 2
    ...
    def test_date_op_in_chain(self):
        ...
        df = pd.DataFrame({"dt": ["2024-03-15", "2024-07-20"]})
        result = TransformationChain(df).date_op("dt", "extract", part="year").result()
        assert result["dt_year"].tolist() == [2024, 2024]

    def test_compute_in_chain(self):
        """Test compute (add_computed_column) in chain."""
        df = pd.DataFrame({"a": [1, 2, 3], "b": [4, 5, 6]})
        result = TransformationChain(df).compute("a + b", "total").result()
    ...
    def test_empty_dataframe_chain(self):
        ...
        result = TransformationChain(df).with_column("b", 1).result()
        assert len(result) == 0
