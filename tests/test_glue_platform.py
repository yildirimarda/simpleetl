...
# ---------------------------------------------------------------------------
# Tests for GlueCatalogHook (config-driven catalog registration)
# ---------------------------------------------------------------------------
...
class TestGlueCatalogHook:
    ...
    def test_hook_skips_when_data_is_none(self, mock_glue_context):
        from simpleetl.formats.glue_catalog import GlueCatalogHook
        ...
        ctx.data = None
        hook.execute(ctx)
        # No exception raised; nothing written because data is None

    def test_hook_writes_dynamic_frame(self, mock_glue_context, caplog):
        from simpleetl.formats.glue_catalog import GlueCatalogHook
