...
def chunk_iterator(df: pd.DataFrame, chunk_size: int) -> Iterator[pd.DataFrame]:
    """Yield successive chunks of a DataFrame."""
    for start in range(0, len(df), chunk_size):
        yield df.iloc[start : start + chunk_size].reset_index(drop=True)


def measure_time_and_memory(func: callable, *args: object, **kwargs: object) -> tuple:
...
def benchmark_csv(df: pd.DataFrame, chunk_size: int) -> dict:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        t1, m1 = measure_time_and_memory(
            lambda: CSVWriter().write(df, path, engine="pandas")
        )
        t2, m2 = measure_time_and_memory(
            lambda: CSVReader().read(path, engine="pandas")
        )
        ...
        t1, m1 = measure_time_and_memory(
            lambda: CSVWriter().write_chunks(
                chunk_iterator(df, chunk_size), path, engine="pandas"
            )
        )
        chunks = []
        t2, m2 = measure_time_and_memory(
            lambda: chunks.extend(
                CSVReader().read_chunks(path, chunk_size, engine="pandas")
            )
        )
        chunked_time = t1 + t2
...
def benchmark_json(df: pd.DataFrame, chunk_size: int) -> dict:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Non-chunked
        t1, m1 = measure_time_and_memory(lambda: JSONWriter().write(df, path))
        t2, m2 = measure_time_and_memory(
        ...
        chunk_paths = [
            os.path.join(tmpdir, f"chunk_{i}.json") for i in range(len(chunks_list))
        ]
        t1, m1 = measure_time_and_memory(
            lambda: [
        ...
        t2, m2 = measure_time_and_memory(
            lambda: results.extend(
                JSONReader().read(p, orient="records", lines=True) for p in chunk_paths
            )
        )
        chunked_time = t1 + t2
        chunked_mem = max(m1, m2)
...
def benchmark_parquet(df: pd.DataFrame, chunk_size: int) -> dict:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Non-chunked
        t1, m1 = measure_time_and_memory(
            lambda: ParquetWriter().write(df, path, engine="pandas")
        )
        t2, m2 = measure_time_and_memory(
            lambda: ParquetReader().read(path, engine="pandas")
        )
        non_chunked_time = t1 + t2
        non_chunked_mem = max(m1, m2)
        ...
        t2, m2 = measure_time_and_memory(
            lambda: chunks.extend(
                ParquetReader().read_chunks(path, chunk_size, engine="pandas")
            )
        )
        chunked_time = t1 + t2
        chunked_mem = max(m1, m2)
...
if __name__ == "__main__":
    ...
    for size in [100_000, 500_000]:
        ...
        print(f"\n{'=' * 60}")
        print(
            f"Dataset: {size:,} rows, {len(df.columns)} columns, "
